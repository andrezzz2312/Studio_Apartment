 ...
 * Tue Jul 12 2022 10:41:26 GMT-0500 (Colombia Standard Time)
...
var sofa, sofaValue, floorValue, objectList, listPosition, shel_1, shelveValue, shel_2, sofaPrice, shel_3, floorPrice, interseccion, moving, piso_1, shelvePrice, shelveClicked, piso_2, mouseMove, posicion, recliner, piso_3, sofaClicked, cameraX, floorClicked, cameraY, cameraZ, info, rotation_recliner, title, delete_recliner, line, screen_shot, subTitle, dowload, upload, json, sofa_1, sofa_2, sofa_3, pdf, my_3dobject, getSofaPosition, total;
...
// pickedObject2 puzzle
function pickedObject2(option, point_option, normal_option){
    var coordSystem = getCoordSystem();
    switch(option){
        case 'OBJECT':
            return _pGlob.pickedObject2;
        case 'POINT':
            if(_pGlob.pickedPoint){
                var pickedPoint = new v3d.Vector3().copy(_pGlob.pickedPoint);
                pickedPoint = coordsTransform(pickedPoint, 'Y_UP_RIGHT', coordSystem)
                switch(point_option){
                    case 'X':
                        return pickedPoint.x;
                    case 'Y':
                        return pickedPoint.y;
                    case 'Z':
                        return pickedPoint.z;
                    case 'XYZ':
                        return pickedPoint.toArray();
                }
            }
            break;
        case 'NORMAL':
            if(_pGlob.pickedNormal){
                var pickedNormal = new v3d.Vector3().copy(_pGlob.pickedNormal);
                pickedNormal = coordsTransform(pickedNormal, 'Y_UP_RIGHT', coordSystem)
...
eventHTMLElem('click', sofa_1, true, function(event) {
  ...
  replaceTexture('M_SofaWhite', 'T_Sofa_White.jpg', './T_Sofa_Blue.jpg',
              function() {}, false);
  sofaValue = 'Blue';
  sofaClicked = true;
  shelveClicked = false;
...
eventHTMLElem('click', sofa_2, true, function(event) {
  ...
  replaceTexture('M_SofaWhite', 'T_Sofa_White.jpg', './T_Sofa_Cream.jpg',
              function() {}, false);
  sofaValue = 'Cream';
  sofaClicked = true;
  shelveClicked = false;
...
eventHTMLElem('click', sofa_3, true, function(event) {
  ...
  replaceTexture('M_SofaWhite', 'T_Sofa_White.jpg', './T_Sofa_White.jpg',
              function() {}, false);
  sofaValue = 'White';
  sofaClicked = true;
  shelveClicked = false;
...
total = [sofaPrice, floorPrice, shelvePrice].reduce(function(x, y) {return x + y;});
...
createObject('CIRCLE', 'circulo', 0, 0, 0, 0.15, 0, 32, 'PERSPECTIVE', 'AMBIENT', 0);
setObjTransform('circulo', false, 'position', [2, 1, 0.05], false);
setObjTransform('circulo', false, 'rotation', [90, '', ''], false);
eventHTMLElem('mousemove', ['WINDOW'], false, function(event) {
  interseccion = raycast(getActiveCamera(), getCameraDirection(true, false), 'SM_Floor');
  if (!(typeof interseccion == 'object' ? !Object.keys(interseccion).length : true)) {
    posicion = dictGet(interseccion[0], 'point');
    setObjTransform('circulo', false, 'position', [getVectorValue(posicion, 'X'), getVectorValue(posicion, 'Y'), 0.01], false);
  }
});
moving = false;
...
registerOnClick2({
        objSelector: 'Navigation',
        xRay: false,
        doubleClick: false,
        mouseButtons: '0,1,2',
        delay: 0.2,
        cbDo:function() {  cameraX = getCameraDirection(true, false)[0];
  cameraY = getCameraDirection(true, false)[1];
  cameraZ = getCameraDirection(true, false)[2];
  tweenCamera(pickedObject2('POINT','XYZ','null'), [cameraX, cameraY, cameraZ], 2, function() {}, 0);
  getJSFunction('vanishTitle')();
...
eventHTMLElem('click', shel_1, true, function(event) {
  ...
  replaceTexture('M_Kitchen_Anaquel', 'T_Anaquel_Salmon.jpg', './T_Anaquel_Salmon.jpg',
              function() {}, false);
  shelveValue = 'Salmon';
  shelveClicked = true;
  sofaClicked = false;
...
eventHTMLElem('click', shel_2, true, function(event) {
  ...
  shelveValue = 'Black';
  replaceTexture('M_Kitchen_Anaquel', 'T_Anaquel_Salmon.jpg', './T_Anaquel_Black.jpg',
              function() {}, false);
  shelveClicked = true;
  sofaClicked = false;
  floorClicked = false;
...
eventHTMLElem('click', shel_3, true, function(event) {
  ...
  replaceTexture('M_Kitchen_Anaquel', 'T_Anaquel_Salmon.jpg', './T_Anaquel_Brown.jpg',
              function() {}, false);
  shelveValue = 'Brown';
  shelveClicked = true;
  sofaClicked = false;
